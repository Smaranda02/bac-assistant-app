...
export default async function SubjectPage({ params }: { params: { subject: string } }) {
  ...
  const { data: subject, error } = await supabase
      ...
      {materials && materials.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {materials.map((material) => (
            ...
            >
              <h3 className="text-xl font-semibold text-gray-800">{material.SubjectChapters?.name || "No chapter name"}</h3>
               <Link href={`/${subjectId}/content/${material.SubjectChapters?.id}`}>
                <p className="text-blue-600 hover:underline mt-4 block">Vizualizeaza continutul pentru acest capitol</p>
               </Link>
            </div>
          ))}
        </div>
    ...
    <h1 className="mt-20 text-3xl font-bold mb-6">Tests for Subject</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {tests.map((test) => (
            ...
            >
              ...
              <Link href={`/student/take-test/${test.id}`}>
                <p className="text-blue-600 hover:underline mt-4 block">Vizualizeaza Test</p>
              </Link>
            </div>
          ))}
        </div>
